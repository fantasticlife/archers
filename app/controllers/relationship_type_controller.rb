class RelationshipTypeController < ApplicationController
  ...
  def show
    ...
    @relationship_type = RelationshipType.find( relationship_type )
  ...
  def update
    ...
    @relationship_type = RelationshipType.find( relationship_type_to_update )
    if @relationship_type.update_attributes( params[:relationship_type] )
      flash[:notice] = "Relationship type details updated"
      redirect_to relationship_type_show_url( :relationship_type => @relationship_type )
    else
      @page_title = "#{@relationship_type.label} - Edit"
      @section = 'relationship'
      @sub_section = 'type'
